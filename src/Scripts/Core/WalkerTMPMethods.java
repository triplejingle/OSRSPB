 ...
 */
public class WalkerTMPMethods extends ClientAccessor{

    public WalkerTMPMethods(ClientContext ctx) {
        super(ctx);
    }
     ...
     *
     * @param t The tile ExchangeToAubury being traversed.
    ...
    public Tile getNextTile(Tile[] t) {
        ...
        int index = 0;                                                      //The index at which the next tile (by our definition) is at. EnvironmentDefault to 0 (start tile).
        final org.powerbot.script.rt4.Player p = ctx.players.local();

        /*
         * Loop through the ExchangeToAubury, backwards.
        ...
        for (int i = t.length - 1; i >= 0; i--) {
            if (t[i].equals(nextTile)) {                                    //This is the index at which the suggested next tile resides
                if (i + 1 <= t.length - 1 && nextTile.distanceTo(p) < 3) {  //If we're not at the end of the ExchangeToAubury and we're very close to the suggested next tile
    ...
    /**
     * Will detect which obstacle to tackle to give WalkerTMPMethods the ability to
                        ...
                        if (calcDist < distance && reachable(go)) {
                            for (String s : go.actions()) {
                                if (s != null && !s.equals("null") && s.contains("Climb-up")) {
                                    obstacle = go;
                                    distance = calcDist;                                                            //Set the distance to the door so we can compare future objects against it to determine the best.
                                    break;
                                }
                            }
                        }
                    ...
                    for (String s : go.actions()) {
                        if (go.tile().distanceTo(nextTile) + go.tile().distanceTo(p) < distance && reachable(go)) {
                            if (s != null && !s.equals("null") && (s.contains("Climb-up") || s.contains("Climb-down") || s.contains("Enter"))) {
                                obstacle = go;
                                distance = go.tile().distanceTo(nextTile) + go.tile().distanceTo(p);              //Set the distance to the door so we can compare future objects against it to determine the best.
                                break;
                            }
                        }
                    }
    ...
    /**
     * Determines if a game door is reachable by
     * checking the tiles around it.
     *
     * @param go The game door being tested.
    ...
    /**
     * Handles the period after interacting with the door.
     *
    ...
    /**
     * Moves a single step towards the destination of a given ExchangeToAubury.
     * Most obstacles such as gates, ladders, doors, and stairs are handled.
     *
     * @param t The tile ExchangeToAubury to be traversed.
     * @return True if a step was taken, or door handled.
     */
    public boolean walkPath(Tile[] t) {
        if(!ctx.players.local().inMotion()||ctx.players.local().tile().distanceTo(ctx.movement.destination().tile())<5) {
    ...
    /**
     * Gets the bounds for a given door
     *
     * @param go The door to get bounds for.
     * @return Bounds. If not found, default bounds returned.
     */
    private int[] getBounds(GameObject go) {
        int tmp = go.orientation();
...
//                        return new int[]{10, 80, 0, -80, 15, 0};
     ...
     *
     * @param s The string to check (name of door?)
    ...
    /**
     * Reverses the given ExchangeToAubury, and then calls {@link #walkPath(Tile[] t)}.
     *
     * @param t
     * @return
     */
